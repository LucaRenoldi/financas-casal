...
export default function AuthPage() {
  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      toast.error(err.code === 'auth/invalid-credential' ? 'E-mail ou senha incorretos' : err.message)
    }
    setLoading(false)
  }
  ...
  async function handleSignup(e) {
    ...
    try {
      ...
      await setDoc(doc(db, 'profiles', user.uid), {
        name, avatarInitials: initials, coupleCode: code, partnerId: null,
        createdAt: new Date().toISOString(),
      })
      toast.success('Conta criada com sucesso! 🎉')
    } catch (err) {
      toast.error(err.code === 'auth/email-already-in-use' ? 'E-mail já cadastrado' : err.message)
    }
    setLoading(false)
  }
      ...
      <div className={styles.card}>
        ...
        {mode === 'login' && (
          <form onSubmit={handleLogin} className={styles.form}>
            ...
            <button type="button" className={styles.forgotLink} onClick={() => setMode('forgot')}>Esqueci minha senha</button>
        ...
        {mode === 'forgot' && (
          <form onSubmit={handleForgot} className={styles.form}>
            <p className={styles.forgotDesc}>Digite seu e-mail e enviaremos um link para criar uma nova senha.</p>
            <Field label="E-mail" type="email" value={form.email} onChange={v => set('email', v)} placeholder="seu@email.com" autoComplete="email" />
            <SubmitBtn loading={loading}>Enviar link de recuperação</SubmitBtn>
            <button type="button" className={styles.backLink} onClick={() => setMode('login')}>← Voltar para o login</button>
          </form>
        )}
      </div>
    </div>
    ...
    <div style={{ marginBottom: 14 }}>
      ...
      <div style={{ position:'relative' }}>
        <input
          ...
          style={{ width:'100%', padding:'13px 16px', paddingRight: isPass ? 44 : 16, background:'var(--bg3)', border:'1px solid var(--bd2)', borderRadius:'var(--r-sm)', fontSize:15, color:'var(--tx1)', transition:'border-color .15s' }}
...
function SubmitBtn({ loading, children }) {
  return (
    <button type="submit" disabled={loading} style={{ width:'100%', padding:'14px', marginTop:8, background: loading ? 'var(--bg4)' : 'var(--green)', color: loading ? 'var(--tx2)' : '#000', border:'none', borderRadius:'var(--r-sm)', fontSize:15, fontWeight:700, transition:'all .15s', cursor: loading ? 'not-allowed' : 'pointer' }}>
